...
import { GoHomeButton } from '@/components/hero/GoHomeButton'
...
export function ChatExperience({
  ...
  const userMessage: UIMessage | undefined = messages.find((m) => m.role === 'user')
  const assistant: UIMessage | undefined = messages.find((m) => m.role === 'assistant')
  const model = useMemo(
  ...
  const userPromptText =
    userMessage?.parts.map((p) => (p.type === 'text' && 'text' in p ? p.text : '')).join('') ||
    initialPrompt
  ...
  return (
    <div className="mx-auto flex h-svh w-full max-w-2xl flex-col gap-6 overflow-y-auto px-4 py-8 sm:py-12">
      <header className="border-border/30 bg-background/85 sticky top-0 z-10 -mx-4 flex items-center justify-between gap-4 border-b px-4 py-3 backdrop-blur-md">
        <p className="text-muted-foreground line-clamp-2 flex-1 text-sm">{userPromptText}</p>
        <div className="flex items-center gap-2">
          <GoHomeButton onGoHome={handleReset} />
      ...
      {model ? (
        <div className="flex flex-col gap-4">
          <ReasoningAccordion text={model.reasoningText} isStreaming={isStreaming} />
          ...
          <div className="flex flex-col gap-6">
            {model.inOrderParts.map((part) => {
              ...
              const isOutput = part.state === 'output-available' && part.output != null
              ...
              return (
                <AnimatePresence key={part.key} mode="wait" initial={false}>
                  {isOutput ? (
                    ...
                    >
                      <ProjectCard {...toProjectCardProps(part.output)} variant="compact" />
                    </motion.div>
                  ) : (
                    <motion.div
                      key="skel"
...
function toProjectCardProps(output: unknown): ProjectCardProps {
  ...
  return {
    ...
    tech_stack: Array.isArray(o.tech_stack) ? (o.tech_stack as string[]) : undefined,
