...
function buildBriefAddendum(job_brief: JobBrief): string {
  return `\n\n${briefAsSystemContext(job_brief)}\n\nBRIEF-MODE WORKFLOW (overrides the default workflow for this turn):\n1. Assume the visitor is evaluating Federico for this role. Do not say "I can't open URLs" — the brief above is the URL's contents, already extracted.\n2. Retrieval chain — follow in order, stop as soon as you get hits:\n   a. Call search_projects with a focused query built from the brief's "Problems the role must solve".\n   b. If 2a returns zero results, call search_projects again with a BROADER query — just the brief's role (e.g. "product designer", "AI engineer", "frontend").\n   c. If 2b ALSO returns zero results, call list_published_projects(limit=6) to enumerate the catalog. Do not skip steps 2a/2b — the fallback is last resort, not first call.\n3. Call show_project_card on the 1–3 strongest matches from whichever step produced results.\n4. Write ONE short paragraph (≤4 sentences) that explicitly connects Federico's work to the brief's role, problems, and outcomes. Name the company from the brief at least once. If you reached step 2c (catalog fallback), open with honesty — e.g. "The portfolio index didn't have a tight match for <X from brief>, so here are the closest projects in the catalog" — and frame the recommendations as "closest existing work", not as direct matches.\n5. End with one concrete question that helps the visitor go deeper (e.g. "Want me to dig into the metrics on <project>?").`;
}
